...
class mobikwikpg extends PaymentModule
{
    ...
    const FOOTER = 2;
    const DISABLE = -1;
    const SANDBOX_SECRET_KEY = '0678056d96914a8583fb518caf42828a'; 
    const SANDBOX_MERCHANT_IDENTIFIER = 'b19e8f103bce406cbd3476431b6b7973';     
    ...
    public function __construct()
    {
        ...
        $this->displayName =$this->l('Mobikwik Payment Gateway');
        $this->description = $this->l('Pay Using Mobikwik Payment Gateway');
    ...
    public function install()
    {
        ...
        if ( !parent::install() 
            ...
            OR !Configuration::updateValue('MOBIKWIKPG_GATEWAY_MODE', '')        
              )
        {            
            return false;
        }
    ...
    public function uninstall()
    {
        ...
        return ( parent::uninstall() 
            ...
            AND Configuration::deleteByName('MOBIKWIKPG_GATEWAY_MODE') 
    ...
    /* Display settings form */
        ...
        <form action="'.$_SERVER['REQUEST_URI'].'" method="post">
          <fieldset>
          <legend><img src="'.__PS_BASE_URI__.'modules/mobikwikpg/logo.png" /></legend>
            <p>'.$this->l('Use the "Test" mode to test out the module. Use the "Live" mode if no problems arise in test mode. Remember to insert your Merchant Identifier and secret key for the live mode.').'</p>
            <p><h2><b>&nbsp'.$this->l('Settings').'</b></h2></p>
            <label>
              '.$this->l('Mode').'
            </label>
            ...
            <p>'.$this->l('Find your Merchant Identifier and Secret Key in your Mobikwik account.').'</p>
            <label>
        ...
        <fieldset>
          ...
          <p>- '.$this->l('Please provide Merchant Identifier and Secret Key provided by Mobikwik').'</p>         
        </fieldset>
        </div>'; 
    ...
    {   
        ...
        {
            $orderId = 'ZPpresta'.rand(200,30000000).'-'.$cart->id;
        ...
        $data['info']['returnUrl'] = $this->context->link->getPageLink( 'order-confirmation', null, null, 'key='.$cart->secure_key.'&id_cart='.(int)($cart->id).'&id_module='.(int)($this->id));
